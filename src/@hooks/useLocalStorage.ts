...
export const useLocalStorage = <T>(key: string, initialValue: T, options? : LocalStorageOptions<T>) => {
    const [storedValue, setStoredValue] = useState<T>(() => {
        ...
        if (item) {
            ...
            if (!options?.overwriteExistingItem || !options.overwriteExistingItem(parsedItem, initialValue))
                return parsedItem;
        }
